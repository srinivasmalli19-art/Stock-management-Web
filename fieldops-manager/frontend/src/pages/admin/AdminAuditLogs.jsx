...
export default function AdminAuditLogs() {
  const [filters, setFilters] = useState({ from: "", to: "", action: "", entityType: "", role: "" });
  const [page, setPage] = useState(1);
  const [selected, setSelected] = useState(null);

  const setFilter = (key, val) => { setFilters((f) => ({ ...f, [key]: val })); setPage(1); };
  const clearFilters = () => { setFilters({ from: "", to: "", action: "", entityType: "", role: "" }); setPage(1); };
  ...
  const { data: res, isLoading, isFetching } = useQuery({
    queryKey: ["audit-logs-admin", activeParams],
  ...
  const handleExport = () => {
    ...
    const headers = ["Date/Time", "User", "Role", "Action", "Entity Type", "Entity ID", "IP Address"];
    ...
    triggerDownload(buildCsvBlob(headers, rows), `audit-logs-${todayStr()}.csv`);
    ...
    <div>
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-muted mt-0.5">Track all user actions in your organisation</p>
        </div>
        ...
        {selected && (
          <div className="space-y-4 text-sm">
            <div className="grid grid-cols-2 gap-y-3 gap-x-4 bg-gray-50 rounded-lg p-4">
              ...
              <div>
                <div className="text-xs text-muted mb-0.5">Entity Type</div>
